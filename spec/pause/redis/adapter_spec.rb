...
describe Pause::Redis::Adapter do
  ...
  describe "#block" do
    ...
    it "saves ip to redis with expiration" do
      ...
      expect(redis_conn.get(blocked_key)).to_not be nil
  ...
  describe "#blocked?" do
    ...
    it "should return true if blocked" do
      adapter.rate_limit!(key, ttl)
      expect(!!redis_conn.get(blocked_key)).to eq(adapter.rate_limited?(key))
    end
  end
  ...
  describe '#rate_limit!' do
    it 'rate limits a key for a specific ttl' do
      expect(adapter.rate_limited?('1')).to be false
      adapter.rate_limit!('1', 10)
      expect(adapter.rate_limited?('1')).to be true
    end
  end
